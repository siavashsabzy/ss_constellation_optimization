...
def ss_coe2eci(coe): # transformation from classical orbital elemens to inertial vectors
    ...
    x_dot = ((x * h * e) / (r * p)) * np.sin(ta) - ((h / r) * (np.cos(raan) * np.sin(aop + ta) + np.sin(raan) * np.cos(aop + ta) * np.cos(i)))  # Velocity X-component
    y_dot = ((y * h * e) / (r * p)) * np.sin(ta) - ((h / r) * (np.sin(raan) * np.sin(aop + ta) - np.cos(raan) * np.cos(aop + ta) * np.cos(i)))  # Velocity Y-component
    z_dot = ((z * h * e) / (r * p)) * np.sin(ta) + ((h / r) * (np.sin(i) * np.cos(aop + ta)))  # Velocity Z-component
    eci_vector = np.array([x, y, z, x_dot, y_dot, z_dot])  # Put X,Y,Z, X_dot, Y_dot, Z_dot into an array to create a vector
    return(eci_vector)
...
def ss_walker(num_plane, num_sat, F, refsat):
    ...
    for i in range(num_plane):
        for j in range(num_sat):
            ...
            if ta >= 180:
                ta -= 360
            sats[(i*num_sat)+j][:] = np.append(refsat[0:6],np.array([refsat[6], refsat[7], refsat[8], raan, refsat[10], ta]))
    return sats
    ...
    for i in range(int(m)):
        ...
        for j in  range(int(mp)):
            ...
            if j == 0 and i == 0:
                ...
                r = np.append( r , [np.array([Re * np.sin(t) * np.cos(p),Re * np.sin(t) * np.sin(p),Re * np.cos(t)])],axis=0)
    return r
